@extends('layouts.app')

@section('content')
<div class="container">
    <div class="columns is-centered">
        <div class="column is-four-fifths box">
          <form method="POST" action="{{ route('register') }}">
            <h1 class="is-size-3 message-header">
                Register ( {{ app('request')->get('status') }} )
            </h1> <br>
              @csrf
              <div class="columns is-multiline">

                <div class="column is-{{ app('request')->get('status') == "santri" ? "4" : "6" }}">
                  <div class="field">
                    <label for="" class="label">Nama</label>
                    <div class="control">
                      <input type="text" class="input @error('name') is-danger @enderror" placeholder="Enter your name" name="name" value="{{ old('name') }}" required>
                        @error('name')
                          <p class="help is-danger">{{ $message }}</p>
                        @enderror
                      </div>
                    </div>
                </div>
                @if ( app('request')->get('status') == "santri" )
                  <div class="column is-4">
                    <div class="field">
                      <label for="" class="label">NIS (Nomor Induk Santri)</label>
                      <div class="control">
                        <input type="text" class="input @error('nis') is-danger @enderror" placeholder="Enter your NIS" name="nis" value="{{ old('nis') }}" required>
                          @error('nis')
                            <p class="help is-danger">{{ $message }}</p>
                          @enderror
                        </div>
                      </div>
                    </div>
                @endif

                <div class="column is-{{ app('request')->get('status') == "santri" ? "4" : "6" }}">
                  <div class="field">
                    <label for="" class="label">Email</label>
                    <div class="control">
                      <input type="email" class="input @error('email') is-danger @enderror" placeholder="Enter an email address" name="email" value="{{ old('email') }}" required>
                        @error('email')
                          <p class="help is-danger">{{ $message }}</p>
                        @enderror
                      </div>
                    </div>
                </div>

                <div class="column is-4">
                  <div class="field">
                    <label for="" class="label">Alamat</label>
                    <div class="control">
                      <input type="text" class="input @error('address') is-danger @enderror" placeholder="Enter your address" name="address" value="{{ old('address') }}" required>
                        @error('address')
                          <p class="help is-danger">{{ $message }}</p>
                        @enderror
                      </div>
                    </div>
                </div>

                <div class="column is-4">
                  <div class="field">
                    <label for="" class="label">Nomor Telpon</label>
                    <div class="control">
                      <input type="text" class="input @error('phone') is-danger @enderror" placeholder="Enter your phone" name="phone" value="{{ old('phone') }}" required>
                        @error('phone')
                          <p class="help is-danger">{{ $message }}</p>
                        @enderror
                      </div>
                    </div>
                </div>

                <div class="column is-4">
                  <label for="" class="label">Jenis Kelamin</label>
                  <p class="control select is-fullwidth">
                    <select name="gender">
                        <option value="">Pilih Jenis Kelamin</option>
                        <option value="laki_laki">Laki-laki</option>
                        <option value="perempuan">Perempuan</option>
                    </select>
                  </p>
                </div>

                <div class="column is-6">
                  <div class="field">
                    <label for="" class="label">Kata Sandi</label>
                    <div class="control">
                      <input type="password" class="input @error('password') is-danger @enderror" placeholder="Enter your password" name="password" value="{{ old('password') }}" required>
                        @error('password')
                          <p class="help is-danger">{{ $message }}</p>
                        @enderror
                      </div>
                    </div>
                </div>

                <div class="column is-6">
                  <div class="field">
                    <label for="" class="label">Konfirmasi Kata Sandi</label>
                    <div class="control">
                      <input type="password" class="input @error('password_confirmation') is-danger @enderror" placeholder="Confirm your password" name="password_confirmation" value="{{ old('password_confirmation') }}" required>
                        @error('password_confirmation')
                          <p class="help is-danger">{{ $message }}</p>
                        @enderror
                      </div>
                    </div>
                </div>

              </div>

              <input type="hidden" name="status" value="{{ app('request')->input('status') }}">
              <a href="/" class="button is-warning">
                Kembali
              </a>
              <button type="submit" class="button is-primary is-right">
                {{ __('Register') }}
              </button>
              {{-- <div class="form-group row mb-0">
                <div class="col-md-6 offset-md-4">
                </div>
              </div> --}}
            </form>
        </div>
    </div>
</div>
@endsection
